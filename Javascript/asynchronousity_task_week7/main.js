...
.then (data => {
    ...
    data.results.forEach( (d)=> {
        ...
        li.innerHTML = `<a href = '#' onClick = "pokeMan('${d.name}','${d.url}')">${d.name}</a>`
        ul.appendChild(li)
    })
    document.getElementById('results').appendChild(ul)
})

function pokeMan (name,url) {
    
    fetch(url)
        .then ( response => response.json())
        .then (data => {
            let {base_experience, height, id, order, weight, abilities, forms, species, types} = data
